...
void AudioAnalyser::analyseAudio(unsigned int _binCount, unsigned int _instantSize)
{
	...
	m_frequencyBeatDetector.detectOutliers(m_fftResult.m_magnitudes, actualInstantSize, 50, 32);
	std::cout << m_sampleCount << " samples in short list\n";
	std::cout << m_srcSampleCount << " samples in full list\n";

	saveFrequencySelectedBeats();
}

void AudioAnalyser::saveFrequencySelectedBeats()
{
	const std::vector<FrequencyOutlier>& beats = m_frequencyBeatDetector.getOutliers();

	FileWriter writer;
	writer.open("output/frequency.beat");
	//Write number of outlier instants
	writer.write(std::to_string(beats.size()).c_str());
	writer.newLine();

	//For each outlier instant
	for (unsigned int i = 0; i < beats.size(); i++)
	{
		//Write index of instant
		writer.write(std::to_string(beats.at(i).m_instant).c_str());
		writer.newLine();

		unsigned int outlyingSubbands = beats.at(i).m_subbands.size();
		//Write number of outlying subbands
		writer.write(std::to_string(outlyingSubbands).c_str());
		writer.newLine();

		//For each outlying subband
		for (unsigned int j = 0; j < outlyingSubbands; j++)
		{
			//Write the subband's index
			writer.write(std::to_string(beats.at(i).m_subbands.at(j)).c_str());
			writer.newLine();

			//Write the subband's outlier threshold
			writer.write(std::to_string(beats.at(i).m_thresholds.at(j)).c_str());
			writer.newLine();
		}
	}
...
const FFTResult& AudioAnalyser::getFFTResult() const
{
	return m_fftResult;
}
...
const OutlierDetector& AudioAnalyser::getOutlierDetector() const
{
	return m_beatDetector;
}

const FFTOutlierDetector& AudioAnalyser::getFFTOutlierDetector() const
{
	return m_frequencyBeatDetector;
}
